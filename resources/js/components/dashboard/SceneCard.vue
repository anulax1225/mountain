    ...
    <CardContent class="space-y-4">
      <div v-if="scene.images?.length > 0" class="gap-2 grid grid-cols-2">
        ...
        <div
          ...
          class="flex justify-center items-center bg-muted rounded-lg aspect-square"
        >
          <span class="font-medium text-muted-foreground">+{{ scene.images.length - 4 }}</span>
        </div>
      </div>

      <div v-else class="flex flex-col justify-center items-center bg-muted/50 py-8 rounded-lg">
        <ImageIcon class="mb-2 w-8 h-8 text-muted-foreground" />
