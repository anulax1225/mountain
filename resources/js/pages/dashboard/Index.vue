...
<template>
  <DashboardLayout :auth="auth">
    <div class="mx-auto max-w-7xl">
      <div class="mb-8">
        <h1 class="font-bold text-zinc-900 text-3xl">Projets</h1>
        <p class="mt-1 text-zinc-600">Gérez vos projets de visite virtuelle</p>
      </div>
      ...
      <div v-else class="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        <Card v-for="project in projects" :key="project.slug" class="hover:shadow-lg transition-shadow cursor-pointer">
          ...
          <CardContent>
            <div class="flex justify-center items-center bg-gradient-to-br from-zinc-100 to-zinc-200 mb-4 rounded-lg aspect-video">
              <svg class="w-12 h-12 text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/>
              </svg>
            </div>
            <div class="flex justify-between items-center text-sm">
              <span class="text-zinc-500">Créé {{ new Date(project.created_at).toLocaleDateString() }}</span>
        ...
        <Sheet v-model:open="sheetOpen">
          <SheetTrigger as-child>
            <Card class="border-2 border-zinc-300 hover:border-purple-400 border-dashed transition-colors cursor-pointer">
              <CardContent class="flex flex-col justify-center items-center py-12 h-full">
                ...
                <p class="mb-1 font-medium text-zinc-900 text-sm">Créer un nouveau projet</p>
                <p class="text-zinc-500 text-xs text-center">Commencez à créer votre visite virtuelle</p>
              </CardContent>
            </Card>
          </SheetTrigger>
